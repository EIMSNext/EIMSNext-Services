...
namespace EIMSNext.Service
{
    public class AppInstallService(IResolver resolver) : IAppInstallService
    {
        ...
        public async Task<string> InstallAsync(string appProfileId)
        {
            ...
            List<FormDataPermissionGroupTemplate> permissionGroupTemplates = permissionGroupTemplateRepo.Queryable.Where(x => x.AppTemplateId == appTemplate.Id).ToList();
            ...
            var permissionGroupMap = permissionGroupTemplates.ToDictionary(x => x.Id, _ => permissionGroupRepo.NewId());
            ...
            var permissionGroups = permissionGroupTemplates.Select(permissionGroupTemplate => InitializeInstalledEntity(new FormDataPermissionGroup
            {
                Id = permissionGroupMap[permissionGroupTemplate.Id],
                AppId = newAppId,
                TemplateId = permissionGroupTemplate.Id,
                FormId = formMap.TryGetValue(permissionGroupTemplate.FormTemplateId, out var formDefId) ? formDefId : permissionGroupTemplate.FormTemplateId,
                Name = permissionGroupTemplate.Name,
                Desc = permissionGroupTemplate.Desc,
                Type = permissionGroupTemplate.Type,
                FormDataPermissions = permissionGroupTemplate.FormDataPermissions,
                DataFilter = permissionGroupTemplate.DataFilter,
                FormFieldPermissions = permissionGroupTemplate.FormFieldPermissions,
                Disabled = permissionGroupTemplate.Disabled,
            }, context, now)).ToList();
            ...
            if (permissionGroups.Count > 0) await permissionGroupRepo.InsertAsync(permissionGroups);
