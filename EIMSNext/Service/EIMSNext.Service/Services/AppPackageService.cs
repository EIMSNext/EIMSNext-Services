...
namespace EIMSNext.Service
{
    ...
    public class AppPackageService(IResolver resolver) : IAppPackageService
    {
        ...
        public async Task<AppPackageImportResult> ImportAsync(Stream stream, long fileLength)
        {
            ...
            var authGroupRepo = _resolver.GetRepository<AuthGroupTemplate>();
            var profileRepo = _resolver.GetRepository<AppProfile>();
            ...
            await ReplaceManyAsync(authGroupRepo, manifest.AuthGroups, context, now);
            ...
            await DeleteAsync(authGroupRepo, plan.AuthGroups.StaleIds);
        ...
        private AppPackageManifest BuildManifest(AppProfile profile)
        {
            ...
            return new AppPackageManifest
            {
                ...
                AuthGroups = authGroupRepo.Queryable.Where(x => x.AppTemplateId == templateId && !x.DeleteFlag).ToList().Select(CloneForPackage).ToList(),
            };
        }
        ...
        private static void ValidateManifest(AppPackageManifest manifest)
        {
            ...
            EnsureResources(manifest.AuthGroups, templateId, "AuthGroupTemplate", x => x.AppTemplateId);
            ...
            foreach (var authGroup in manifest.AuthGroups)
            {
                if (!string.IsNullOrWhiteSpace(authGroup.FormTemplateId) && !formIds.Contains(authGroup.FormTemplateId))
                {
                    throw new BadRequestException($"授权组 {authGroup.Id} 引用了不存在的表单模板");
                }
            }
        ...
        private ImportPlan BuildImportPlan(AppPackageManifest manifest)
        {
            ...
            VerifyTargetOwnership(authGroupRepo, manifest.AuthGroups, manifest.Template.Id, x => x.AppTemplateId, "AuthGroupTemplate");
            ...
            return new ImportPlan
            {
                ...
                AuthGroups = BuildState(authGroupRepo.Queryable.Where(x => x.AppTemplateId == manifest.Template.Id), manifest.AuthGroups),
            };
        }
        ...
        private static AppPackagePreview ToPreview(ImportPlan plan)
        {
            return new AppPackagePreview
            {
                ...
                [
                    ...
                    ToResourcePreview("AuthGroupTemplate", plan.AuthGroups),
                ],
            };
        }
        ...
        private sealed class ImportPlan
        {
            ...
            public ResourceState AuthGroups { get; init; } = new();
        }
