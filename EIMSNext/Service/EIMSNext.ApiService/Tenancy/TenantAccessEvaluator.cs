...
namespace EIMSNext.ApiService
{
    public class TenantAccessEvaluator(IResolver resolver) : ApiServiceBase(resolver)
    {
        ...
        // 以减少对 TenantAdminGroup 的重复查询；切勿改为 Singleton，否则会跨请求泄漏用户权限数据。
        ...
        public AdminPermissionSnapshot GetSnapshot()
        {
            ...
            ApplyScope(
                snapshot,
                CombineScope(groups, x => x.AppEmployeeGroupScopeMode, x => x.AppEmployeeGroupIds),
                static (x, mode) => x.AppEmployeeGroupScopeMode = mode,
                static (x, ids) => x.AppEmployeeGroupIds = ids);
            ...
            ApplyScope(
                snapshot,
                CombineScope(groups.Where(x => x.ContactEmployeeGroupPermission >= PermissionLevel.View), x => x.ContactEmployeeGroupScopeMode, x => x.ContactEmployeeGroupIds),
                static (x, mode) => x.ContactViewEmployeeGroupScopeMode = mode,
                static (x, ids) => x.ContactViewEmployeeGroupIds = ids);

            ApplyScope(
                snapshot,
                CombineScope(groups.Where(x => x.ContactEmployeeGroupPermission >= PermissionLevel.Manage), x => x.ContactEmployeeGroupScopeMode, x => x.ContactEmployeeGroupIds),
                static (x, mode) => x.ContactManageEmployeeGroupScopeMode = mode,
                static (x, ids) => x.ContactManageEmployeeGroupIds = ids);
        ...
        public List<string> GetUsageAppIdsForCurrentEmployee()
        {
            ...
            var employeeGroupIds = memberScope.EmployeeGroupIds.ToList();
        ...
        public async Task SyncCreatedAppToNormalTenantAdminGroupsAsync(string appId)
        {
            ...
            var adminGroupService = Resolver.GetService<ITenantAdminGroupService, TenantAdminGroup>();
        ...
        private IReadOnlyList<TenantAdminGroup> LoadNormalGroups()
        {
            ...
            _normalGroups = Resolver.GetService<ITenantAdminGroupService, TenantAdminGroup>()
                .All()
                .Where(x =>
                    x.CorpId == IdentityContext.CurrentCorpId &&
                    !x.DeleteFlag &&
        ...
        private void EnsureEmployeeGroupInScope(string employeeGroupId, string scopeMode, IEnumerable<string> employeeGroupIds, string message)
        {
        ...
        private static PermissionScope CombineScope(
            IEnumerable<TenantAdminGroup> groups,
            Func<TenantAdminGroup, ScopeMode> modeSelector,
            Func<TenantAdminGroup, IEnumerable<string>> idsSelector)
        {
        ...
        private sealed record EmployeeMemberScope(
            string EmployeeId,
            HashSet<string> EmployeeGroupIds,
            ...
            HashSet<string> AncestorDepartmentIds)
        {
            public bool Matches(Member member)
            {
                return (member.Type == MemberType.Employee && member.Id == EmployeeId) ||
                       (member.Type == MemberType.EmployeeGroup && EmployeeGroupIds.Contains(member.Id)) ||
