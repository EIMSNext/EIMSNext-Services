...
namespace EIMSNext.Identity.Host.Controllers
{
    ...
    public class OpenController : ControllerBase
    {
        ...
        public string Ping()
        {
            return "Identity Server is running.";
        }
        ...
        [Route("open/integration/authorize"), HttpGet]
        public async Task<IActionResult> GetIntegrationAuthorizationUrl([FromQuery] string type, [FromQuery] string state, CancellationToken cancellationToken)
        {
            ...
            {
                return BadRequest(new { message = "type 不能为空" });
            }

            var result = await _integrationAuthService.GetAuthorizationUrlAsync(type, state ?? string.Empty, cancellationToken);
            return Ok(new
            {
                result.Type,
                result.DisplayName,
