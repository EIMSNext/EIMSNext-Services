...
namespace EIMSNext.Core.Repository
{
    public abstract class RepositoryBase<T> : IRepository<T> where T : IMongoEntity
    {
        ...
        {
            var idFilter = FilterBuilder.Eq(Constants.Field_BsonId, id);
            session = GetSessionHandle(session);
        ...
        {
            return UpdateCore(FilterBuilder.Eq(Constants.Field_BsonId, id), update, false, upsert, session);
        }
        ...
        {
            return UpdateCoreAsync(FilterBuilder.Eq(Constants.Field_BsonId, id), update, false, upsert, session);
        }
        ...
        {
            return DeleteCore(FilterBuilder.Eq(Constants.Field_BsonId, id), session);
        }
        public virtual DeleteResult Delete(IEnumerable<string> ids, IClientSessionHandle? session = null)
        {
            return DeleteCore(FilterBuilder.In(Constants.Field_BsonId, ids), session);
        }
        ...
        {
            return DeleteCoreAsync(FilterBuilder.Eq(Constants.Field_BsonId, id), session);
        }
        public virtual Task<DeleteResult> DeleteAsync(IEnumerable<string> ids, IClientSessionHandle? session = null)
        {
            return DeleteCoreAsync(FilterBuilder.In(Constants.Field_BsonId, ids), session);
        }
        ...
        {
            return FilterBuilder.Eq(Constants.Field_BsonId, entity.Id);
        }
