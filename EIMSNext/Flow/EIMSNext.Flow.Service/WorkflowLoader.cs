...
namespace EIMSNext.Flow.Service
{
    ...
    {
        private static Type IFlowNodeType = typeof(IFlowNode);
        ...
        public WorkflowLoader(IResolver resolver)
        {
            _resolver = resolver;
            _defService = resolver.Resolve<IWfDefinitionService>();
            _registry = resolver.Resolve<IWorkflowRegistry>();
            _expressionEvaluator = resolver.Resolve<IExpressionEvaluator>();
            _logger = resolver.Resolve<ILogger<WorkflowLoader>>();
        }
        ...
        private WorkflowDefinition Convert(WfMetadata metadata)
        {
            ...
            var result = new WorkflowDefinition
            {
                ...
                Steps = ConvertSteps(metadata.Steps, dataType),
                DefaultErrorBehavior = (WorkflowErrorHandling)metadata.DefaultErrorBehavior,
                DefaultErrorRetryInterval = metadata.DefaultErrorRetryInterval,
                DataType = dataType
            };
        ...
        private WorkflowStepCollection ConvertSteps(ICollection<WfStep> source, Type dataType)
        {
            var result = new WorkflowStepCollection();
            int i = 0;
            var stack = new Stack<WfStep>(source.Reverse());
            var parents = new List<WfStep>();
            var compensatables = new List<WfStep>();
            ...
            while (stack.Count > 0)
            {
                ...
                targetStep.ErrorBehavior = (WorkflowErrorHandling?)nextStep.ErrorBehavior;
                ...
                if (nextStep.Work != null)
                {
                    ...
                    if (nextStep.Work.Count > 0)
                        parents.Add(nextStep);
                }

                if (nextStep.CompensateWith != null)
                {
                    foreach (var compChild in nextStep.CompensateWith.Reverse<WfStep>())
            ...
            foreach (var parent in parents)
            {
                ...
                foreach (var branch in parent.Work)
                {
        ...
        private void AttachInputs(WfStep source, Type dataType, Type stepType, WorkflowStep step)
        {
            if (stepType.IsAssignableTo(IFlowNodeType))
            {
                Expression<Func<IStepBody, WfStep?>> memberExpr = x => (x as IFlowNode)!.Metadata;
                Expression<Func<object, WfStep>> valueExpr = (x => source);

                step.Inputs.Add(new MemberMapParameter(valueExpr, memberExpr));
            }
        ...
        private void AttachOutputs(WfStep source, Type dataType, Type stepType, WorkflowStep step)
        {
        ...
        private void AttachOutcomes(WfStep source, Type dataType, WorkflowStep step)
        {
        ...
        private Action<IStepBody, object, IStepExecutionContext> BuildScalarInputAction(KeyValuePair<string, object> input, PropertyInfo stepProperty)
        {
            ...
            void acn(IStepBody pStep, object pData, IStepExecutionContext pContext)
            {
                ...
                if (stepProperty.PropertyType.IsEnum)
                    stepProperty.SetValue(pStep, resolvedValue == null ? true : Enum.Parse(stepProperty.PropertyType, (string)resolvedValue, true));
                else
                {
                ...
                while (stack.Count > 0)
                {
                    ...
                    foreach (var prop in subobj.Properties().ToList())
                    {
                        if (prop.Name.StartsWith("@"))
                        {
                            ...
                            subobj.Add(prop.Name.TrimStart('@'), resolvedValue == null ? null : JToken.FromObject(resolvedValue));
                        }
                    }
