...
namespace EIMSNext.Identity.Tests
{
    ...
    public class TokenRequestHandlerTests
    {
        ...
        public async Task HandleAsync_UsesClientScopes_WhenScopeMissing()
        {
            ...
            Assert.AreEqual("admin", result.Claims.Single(x => x.Type == IdentityClaimTypes.Id).Value);
            Assert.AreEqual("corp-001", result.Claims.Single(x => x.Type == IdentityClaimTypes.Corp).Value);
        }
        ...
        public async Task HandleAsync_SystemGrant_ReturnsSystemClaims()
        {
            ...
            Assert.IsTrue(result.Succeeded);
            Assert.AreEqual("system", result.Claims.Single(x => x.Type == IdentityClaimTypes.Id).Value);
            Assert.AreEqual("wf_wf-inst-001", result.Claims.Single(x => x.Type == IdentityClaimTypes.Name).Value);
            Assert.AreEqual("corp-001", result.Claims.Single(x => x.Type == IdentityClaimTypes.Corp).Value);
            Assert.AreEqual(IdentityType.System.ToString(), result.Claims.Single(x => x.Type == IdentityClaimTypes.IdentityType).Value);
        ...
        private static IReadOnlyList<ITokenGrantHandler> CreateGrantHandlers(User user, FakeIdentityLoginAuditService? auditLoginService = null)
        {
            var contextAccessor = CreateHttpContextAccessor();
            auditLoginService ??= new FakeIdentityLoginAuditService();
            return
            [
        ...
        private sealed class FakeIdentityLoginAuditService : IIdentityLoginAuditService
        {
