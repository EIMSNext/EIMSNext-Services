...
namespace EIMSNext.Identity.Tests
{
    ...
    public class AccountSecurityServiceTests
    {
        ...
        {
            using var dbContext = new FakeIdentityDbContext(
            [
        ...
        private static AccountSecurityService CreateService(FakeIdentityDbContext dbContext)
        {
        ...
        private sealed class FakeIdentityDbContext : IIdentityDbContext
        {
            ...
            private readonly List<IdentityLoginAudit> _auditLogins = [];
            private readonly List<EIMSNext.Identity.Models.PublicAccessSetting> _publicSettings = [];
            private readonly List<CorporateSettingReadModel> _corporateSettings = [];

            public FakeIdentityDbContext(IEnumerable<User>? users = null)
            {
                _users = users?.ToList() ?? [];
            }
        ...
        private sealed class VerifyIdentityTicketProxy
        {
            ...
            {
                var ticketType = typeof(AccountSecurityService).Assembly.GetType("EIMSNext.Identity.AccountSecurity.VerifyIdentityTicket")!;
