...
using EIMSNext.Auth.Host.Controllers;
...
namespace EIMSNext.Auth.Tests
{
    ...
    public class BuiltInClientRequestPolicyTests
    {
        ...
        public async Task SystemToken_UsesSystemGrantAndPreservesTaskParameters()
        {
            var handler = new RecordingTokenRequestHandler
            {
                NextResult = TokenRequestResult.Success(
                    ...
                    ["api.readwrite"],
                    [
                        new Claim(AuthClaimTypes.Subject, "system"),
                        new Claim(AuthClaimTypes.Name, "wf_instance-001"),
                        new Claim(AuthClaimTypes.Id, "system"),
                        new Claim(AuthClaimTypes.Corp, "corp-001"),
                        new Claim(AuthClaimTypes.IdentityType, IdentityType.System.ToString())
                    ])
            };
        ...
        public async Task PublicToken_UsesPublicClientAndPublicGrant()
        {
            var handler = new RecordingTokenRequestHandler
            {
                NextResult = TokenRequestResult.Success(
                    ...
                    ["DashLink"],
                    [
                        new Claim(AuthClaimTypes.Subject, "public_target"),
                        new Claim(AuthClaimTypes.Name, "public"),
                        new Claim(AuthClaimTypes.Id, "public_target"),
                        new Claim(AuthClaimTypes.Corp, "corp-001")
                    ])
            };
        ...
        public async Task Login_UsesConfiguredWebClientWhenOriginAllowed()
        {
            var handler = new RecordingTokenRequestHandler
            {
                NextResult = TokenRequestResult.Success(
                    ...
                    ["openid"],
                    [
                        new Claim(AuthClaimTypes.Subject, "admin"),
                        new Claim(AuthClaimTypes.Name, "Admin"),
                        new Claim(AuthClaimTypes.Id, "admin"),
                        new Claim(AuthClaimTypes.Corp, "corp-001")
                    ])
            };
        ...
        private sealed class FakeHostEnvironment : IHostEnvironment
        {
            ...
            public string ApplicationName { get; set; } = "EIMSNext.Auth.Tests";
