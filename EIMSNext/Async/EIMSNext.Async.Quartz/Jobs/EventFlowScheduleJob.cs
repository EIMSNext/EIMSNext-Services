...
{
    /// <summary>
    /// 数据流定时调度扫描作业。扫描到到期项后通过 IMessagePublisher 发布 EventFlowRunTaskArgs，
    /// 由 Async.Tasks 中的 EventFlowRunConsumer 调 FlowApiClient.RunEventFlow 触发一次执行。
    /// </summary>
    [DisallowConcurrentExecution]
    public class EventFlowScheduleJob : JobBase<EventFlowScheduleJob>
    {
        public EventFlowScheduleJob(IResolver resolver) : base(resolver)
        {
        }
        ...
        {
            var scheduleRepo = Resolver.GetRepository<EventFlowScheduleItem>();
            ...
            Logger.LogInformation("EventFlow schedule scan found {Count} due items", dueItems.Count);

            foreach (var item in dueItems)
            {
                try
                {
                    var definition = definitionRepo.Get(item.EventFlowId);
                    ...
                    await publisher.PublishAsync(new EventFlowRunTaskArgs
                    {
                ...
                {
                    Logger.LogError(ex, "EventFlow schedule execution failed. ScheduleId={Id}, EventFlowId={EventFlowId}", item.Id, item.EventFlowId);
                }
            }
        }

        private async Task AdvanceAsync(IRepository<EventFlowScheduleItem> scheduleRepo, Wf_Definition definition, EventFlowScheduleItem item)
        {
            var triggerSetting = definition.Metadata.Steps.FirstOrDefault()?.EfNodeSetting?.TriggerSetting;
